...
uint8_t CAN_Transmit(CAN_TypeDef* CANx, CanTxMsg* TxMessage)
{
 ...
 transmit_mailbox = 0;
  ...
  if (transmit_mailbox != CAN_TxStatus_NoMailBox)
  {
    ...
    if (TxMessage->IDE == CAN_Id_Standard)
    {
      ...
      CANx->sTxMailBox[transmit_mailbox].TIR = (((TxMessage->StdId) << 21) |   //此处在库文件的基础上有改动
                                                  TxMessage->RTR);		
    }
    else
    {
 ...
 // if(transmit_mailbox==2)
 // {
 // 	while((CANx->TSR&CAN_TSR_TME0) != CAN_TSR_TME0);
//	while((CANx->TSR&CAN_TSR_TME1) != CAN_TSR_TME1);
//	while((CANx->TSR&CAN_TSR_TME2) != CAN_TSR_TME2);
//	while((CANx->sTxMailBox[transmit_mailbox].TIR & TMIDxR_TXRQ) == TMIDxR_TXRQ);
  //}
  while((CAN1->sTxMailBox[transmit_mailbox].TIR & TMIDxR_TXRQ) == TMIDxR_TXRQ);
  //while(flag==0xff);
